   ...
   switch( iSource )
   {
      case 0:   /* Image from resource by numeric id */
         if( HB_ISNUM( 3 ) && hb_parni( 3 ) == IMAGE_ICON )
            hImage = ( HICON ) LoadImage( ( HINSTANCE ) wvg_hInstance(), MAKEINTRESOURCE( hb_parni( 1 ) ), IMAGE_ICON, hb_parni( 4 ), hb_parni( 5 ), LR_DEFAULTSIZE | LR_SHARED );
         else
            hImage = ( HBITMAP ) LoadImage( ( HINSTANCE ) wvg_hInstance(), MAKEINTRESOURCE( hb_parni( 1 ) ), IMAGE_BITMAP, hb_parni( 4 ), hb_parni( 5 ), LR_DEFAULTSIZE | LR_SHARED );
         break;

      case 1:   /* image from resource by name */
         if( HB_ISNUM( 3 ) && hb_parni( 3 ) == IMAGE_ICON )
            hImage = ( HICON ) LoadImage( ( HINSTANCE ) wvg_hInstance(), HB_PARSTR( 1, &hString, NULL ), IMAGE_ICON, hb_parni( 4 ), hb_parni( 5 ), LR_DEFAULTSIZE | LR_SHARED );
         else
            hImage = ( HBITMAP ) LoadImage( ( HINSTANCE ) wvg_hInstance(), HB_PARSTR( 1, &hString, NULL ), IMAGE_BITMAP, hb_parni( 4 ), hb_parni( 5 ), LR_DEFAULTSIZE | LR_SHARED );
         break;
