...
FUNCTION demo_rmchart()

   LOCAL oRmChart, oCrt1, nOpc := 1, nTemp, oCrt

   oCrt := wvgCrt():New()
   oCrt:lModal := .T.
   oCrt:Create( ,, { 1, 1 }, { 40, 100 } )
   SET WRAP ON
   SetColor( "W/B" )
   CLS
   oRmChart := RMChartClass():New()
   oCrt1 := wvgTstRectangle():New( oCrt, , { 0, -20 }, { -MaxRow() - 1, -MaxCol() + 19 } )
   oCrt1:Create()
   ...
   DO WHILE .T.
      ...
      @ Row() + 2, 1 PROMPT "Select and ENTER"
      MENU TO nOpc
      nTemp := 1
      DO CASE
...
FUNCTION TstRmChart()
   ...
   @ 3, 0 SAY "Click on blue window and ESC to exit"
   Inkey(0)
